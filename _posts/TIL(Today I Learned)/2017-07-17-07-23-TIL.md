...
# 7월 21일 (금)
- 알고리즘 문제를 풀었다.
- 쉘스크립트 자료를 읽었다. [Bash Shell Script](https://www.gitbook.com/book/mug896/shell-script/details) pyenv를 처음 설치 했을때 `$ echo 'eval "$(pyenv virtualenv-init -)"' >> ~/.zshrc` 같은 명령을 정체도 모르고 입력했었는데 이게 쉘스크립트 중의 하나였구나.
- 자료가 좀 어려워서 찾아보니 생활코딩 리눅스 카테고리에 [쉘 스크립트](https://opentutorials.org/course/2598/14204) 강의가 있었다. 들어봐야지.
